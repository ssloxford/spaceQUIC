...
#else
    quic_client_init(&TO_LAB_Global.quic_client, false);
    quic_client_connect(&TO_LAB_Global.quic_client, TO_LAB_Global.tlm_dest_IP, "1235");
    quic_client_setup_epoll(&TO_LAB_Global.quic_client);
#endif
}
    ...
    do {
        ...
        if ((CFE_SB_status == CFE_SUCCESS) && (TO_LAB_Global.suppress_sendto == false)) {
            ...
            if (TO_LAB_Global.downlink_on == true) {
                ...
                uint8_t *enc_buff;
                int enc_size;
                bool must_free = NET_encrypt((const uint8_t *) SBBufPtr, (int) size, &enc_size, &enc_buff, false);

                status = OS_SocketSendTo(TO_LAB_Global.TLMsockid, enc_buff, enc_size, &d_addr);
                OS_printf("INFO: sent TO packet of size %d. size before net_encrypt %zu\n", enc_size, size);
...
#else
                int r = quic_client_write(&TO_LAB_Global.quic_client, (const uint8_t *) SBBufPtr, size);
                OS_printf("quic TO write status: %d\n", r);
#endif
            } else {
                status = 0;
            }
...
#ifdef Quic
    if (TO_LAB_Global.downlink_on == true) {
        int r = quic_client_step(&TO_LAB_Global.quic_client, 10);
        assert(r == 0);
    }
#endif
}
